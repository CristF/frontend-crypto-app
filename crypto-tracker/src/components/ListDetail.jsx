...
import { findListById } from '../utils/api';
    ...
    useEffect(() => {
        const loadList = async () => {
            try {
                ...
                console.log('Raw list data received:', data);
                
                if (!data || !data.cryptos) {
                    throw new Error('Invalid list data received');
                }
                
                // Transform the data with proper market cap and volume
                const transformedData = {
                    ...data,
                    cryptos: data.cryptos.map(crypto => ({
                        _id: crypto.cryptoId, // For unique key prop
                        name: crypto.name,
